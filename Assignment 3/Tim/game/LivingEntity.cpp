//! @file 
//! @brief Implementation file for LivingEntity class
...
LivingEntity::LivingEntity(char c, int strength, int dexterity, int constitution, int intelligence, int wisdom, int charisma, int level, std::string cname) :
	level(level), name(cname), attackBonus(level)
{
	...
	void LivingEntity::updateStats()
	{
		...
		attackBonus = level+getModifier(0);

		// Set damage bonus to strength modifier
		damageBonus = getModifier(0);
	...
	void LivingEntity::equipBasic() {
		...
		equipped[6]=new Item("Dagger", Item::ItemType::WEAPON, 0, 0, 0, 0, 0, 0, 0, 0, 0);
		updateStats();
	}
	...
	void LivingEntity::levelUp(int levels)
	{
	...
	std::string LivingEntity::toString(){
		// Create a string to display attack bonuses and attacks per turn in the form "+x / +y / +z"
		std::string attacks="";
		for (int i = 0; i < attacksPerTurn; i++) {
			attacks += "+" + to_string(attackBonus-5*i);
			if (i < attacksPerTurn-1) {
				attacks += "/";
			}
		...
		return "Name: " + name + "\nLevel: " + to_string(level) + "\nBase:\nSTR: " + std::to_string(getStrength()) + ", DEX: " + std::to_string(getDexterity()) + ", CON: " + std::to_string(getConstitution()) +
			...
			"\nDamage Bonus:" + to_string(damageBonus) + ", Attack bonus:" + attacks +
